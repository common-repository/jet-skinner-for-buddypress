<?php
/* 
Plugin Name: Jet Skinner for BuddyPress
Version: 1.2.5
Plugin URI: http://milordk.ru/r-lichnoe/opyt/cms/vasha-socialnaya-set-s-nastroeniem.html
Description: Provides the ability to apply different skins to a theme based on Skinner plugin of Windy Road (http://windyroad.com)
Author: Jettochkin
Author URI: http://milordk.ru

Copyright (C)2010 Jettochkin
...
function jet_skinner_load_textdomain() {
	$locale = apply_filters( 'bp_locale', get_locale() );
	$mofile = WP_PLUGIN_DIR . "/jet-skinner-for-buddypress/j-skinner-$locale.mo";

	if ( file_exists( $mofile ) )
		load_textdomain( 'j-skinner', $mofile );
}
add_action ( 'plugins_loaded', 'jet_skinner_load_textdomain', 9 );
...
function get_skin_data( $skin_file ) {
	...
	if ( '' == $author_uri[1] ) {
		...
		$author = '<a href="' . trim( $author_uri[1] ) . '" title="' . __('Visit author homepage','j-skinner') . '">' . trim( $author_name[1] ) . '</a>';
	}
