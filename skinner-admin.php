...
function add_skins_editor() {
	if( $_GET[ 'page' ] == 'editskinner-admin.php' ) {
		...
		if ( !current_user_can('edit_themes') )
			wp_die('<p>'.__('You do not have sufficient permissions to edit skins for this blog.','j-skinner').'</p>');
...
function skin_editor() {
	...
	if( count($skins) > 0 ) {
		...
		if( isset($_GET['a']) ) {
			?><div id="message" class="updated fade"><?php
				?><p><?php _e('File edited successfully.','j-skinner') ?></p><?php
			?></div><?php
		}
	 	?><div class="wrap"><?php
			?><form name="theme" action="themes.php?page=editskinner-admin.php" method="post"><?php
				?><input type="hidden" name="skinaction" value="switch" /><?php
					 _e('Select skin to edit:','j-skinner');
					...
					?></select>
			<input type="submit" name="Submit" value="<?php _e('Select &raquo;','j-skinner') ?>" class="button" />
			</form>
			</div>
	...
	<?php
		if ( is_writeable($real_file) ) {
			echo '<h2>' . sprintf(__('Editing <code>%s</code>','j-skinner'), $file_show) . '</h2>';
		} else {
			echo '<h2>' . sprintf(__('Browsing <code>%s</code>','j-skinner'), $file_show) . '</h2>';
		}
	?>
				<div id="templateside">
					<h3><?php printf(__("<strong>'%s'</strong> skin files","j-skinner"), $skin) ?></h3>
	...
	?>
		...
		<?php if ( is_writeable($real_file) ) : ?>
					<p class="submit">
		<?php
				echo "<input type='submit' name='submit' value='	" . __('Update File &raquo;','j-skinner') . "' tabindex='2' />";
		?>
					</p>
		<?php else : ?>
					<p><em><?php _e('If this file were writable you could edit it.','j-skinner'); ?></em></p>
		<?php endif; ?>
				</form>
		<?php
		}
		else {
			echo '<div class="error"><p>' . __('Oops, no such file exists! Double check the name and try again, merci.','j-skinner') . '</p></div>';
		}
	} else {
		// no skins
		?><div class="error"><?php
				?><p><?php _e('Oops, this theme has no skins.','j-skinner') ?></p><?php
			?></div><?php
	}
	?>
...
function skinner_new_skin() {
	...
	if( isset( $_POST[ 'skin' ] ) ) {
		...
		$lname = strtolower($name);
		if( array_search(strtolower($lname), $lnames) ) {
			?><div id="message1" class="error"><p><?php echo __('Please choose another name. An existing skin conflicts with \'','j-skinner') . $title . __('\'.');  ?></p></div><?php			
			skinner_display_new_skin_form( $title );
			return false;	
		}
		if( !is_dir(get_skin_root()) ) {
			if( !mkdir(get_skin_root()) ) {
				?><div id="message2" class="error"><p><?php echo __('A \'skins\' directory could be created in \'' . get_theme_root() . '\'.  Please create this directory manually.','j-skinner');  ?></p></div><?php						
				skinner_display_new_skin_form( $title );
				return false;	
			}
		}
		...
		if( !is_dir($dir) ) {
			if( !mkdir($dir) ) {
				?><div id="message3" class="error"><p><?php echo __('A \'skins/' . $name . '\' directory could be created in \'' . get_theme_root() . '\'.  Please create this directory manually.','j-skinner');  ?></p></div><?php						
				skinner_display_new_skin_form( $title );
				return false;	
			}
		}
...
SKINNEREOD;
			...
			?><div id="message5" class="updated fade"><p><?php echo __('The skin \'' . $title . '\' has been created. ','j-skinner') . '<a href="' . $location . '">Edit &raquo;</a>';  ?></p></div><?php
			skinner_set_skin_status( $name, get_current_theme(), false);
			return true;
		}
		else {
			?><div id="message4" class="error fade"><p><?php echo __('Could not create \'style.css\' file. Please create this file manually.','j-skinner');  ?></p></div><?php						
		}
	}
...
function skinner_display_new_skin_form() {
	?><div class="wrap"><?php
		...
		?><form style="text-align: center" action="themes.php?page=skinner-admin.php" method="post"><?php
			...
			?><p class="submit"><input type="submit" name="createskin" value="<?php _e('Create Skin &raquo;','j-skinner'); ?>" /></p><?php
	  	?></form><?php
...
function skinner_display_msgs() {
	if ( ! validate_current_skin() ) {
		?><div id="message1" class="updated fade"><p><?php _e('The active skin is broken.  Reverting to the default skin.','j-skinner'); ?></p></div><?php
	}
	elseif ( isset($_GET['activated']) ) {
		?><div id="message2" class="updated fade"><p><?php printf(__('New skin activated. <a href="%s">View site &raquo;</a>','j-skinner'), get_bloginfo('home') . '/'); ?></p></div><?php
	}
	if( isset( $_GET['error'])) {
		if( $_GET[ 'error'] == 'style') {
			$theme = current_theme_info();
			$style = $theme->stylesheet_dir . '/skins/' . $_GET[ 'skin' ] . '/style.css'; 
			?><div id="message4" class="error"><p><?php echo sprintf(__('Could not create \'%s\'. Please create this file manually.','j-skinner'), $style );  ?></p></div><?php						
	...
	if( isset( $_GET[ 'action'] ) ) {
		...
		?><div id="message5" class="updated fade"><p><?php echo __('The skin \'' . $_GET[ 'title' ] . '\' has been created. ','j-skinner') . '<a href="' . $location . '">Edit &raquo;</a>';  ?></p></div><?php
	}	
}
...
<?php endif; ?>
<h3><?php printf(__('%1$s %2$s by %3$s','j-skinner'), $ct->title, $ct->version, $ct->author) ; ?></h3>
<p><?php echo $ct->description; ?></p>
<?php if ($ct->parent_skin) { ?>
	<p><?php printf(__('The stylesheet files are located in <code>%2$s</code>.<br/><strong>%3$s</strong> uses files from <strong>%4$s</strong>.  Changes made to %4$s files will affect both skins.','j-skinner'), $ct->title, $ct->stylesheet_dir, $ct->title, $ct->parent_skin); ?></p>
<?php } else { ?>
	<p><?php printf(__('All of this skin&#8217;s files are located in <code>%2$s</code>.','j-skinner'), $ct->title, $ct->stylesheet_dir); ?></p>
<?php } ?>
</div>
<?php
...
<h2><?php _e('Broken skins','j-skinner'); ?></h2>
<p><?php _e('The following skins are installed but incomplete.  skins must have a stylesheet called style.css or style.css.php.','j-skinner'); ?></p>

<table width="100%" cellpadding="3" cellspacing="3">
	<tr>
		<th><?php _e('Name','j-skinner'); ?></th>
		<th><?php _e('Description','j-skinner'); ?></th>
	</tr>
<?php
	$skin = '';
...
?>
<h2><?php _e('Create Your Own Skin','j-skinner'); ?></h2>
<p><?php _e('You can make your own skin for','j-skinner');?> <?php echo($parent); ?>. <?php _e('Creating your own skin is easy and uses the themes default look and feel as a starting point.','j-skinner'); ?></p>
  <form action="themes.php?page=skinner-admin.php" method="post">
	<p class="submit"><input type="submit" name="createskin" value="Create Skin &raquo;" /></p>
  </form>
<?php	if( $archive ) { ?>
<h2><?php _e('Get More Skins','j-skinner'); ?></h2>
<p><?php _e('You can find additional skins for','j-skinner'); ?> <?php echo($parent); ?> <?php _e('in the','j-skinner'); ?> <a href="<?php echo(trim($archive)); ?>"><?php echo($parent); ?> <?php _e('skin directory</a>. To install a skin you generally just need to upload the skin folder into your <code>','j-skinner'); ?> <?php echo $curr_theme->stylesheet_dir; ?><?php _e('/skins</code> directory. Once a skin is uploaded, you should see it on this page.','j-skinner'); ?></p>
